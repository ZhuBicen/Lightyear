...
LRESULT CALLBACK CTrayIcon::MessageProcessorWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == TRAY_WINDOW_MESSAGE)
	{
        if (WM_RBUTTONDOWN == lParam) {
            MLOG(logINFO) << "Receive tray message RBUTTONDOWN = 0x" << WM_RBUTTONDOWN;
        }
...
CTrayIcon::CTrayIcon(const TCHAR* name, bool visible, HICON hIcon, bool destroy_icon_in_destructor)
...
bool CTrayIcon::AddIcon()
{
	...
	size_t tip_len = min(sizeof(data.szTip)-1, wcslen(m_Name.c_str()));
...
void CTrayIcon::SetName(const wchar_t* name)
{
	...
	if (m_Visible)
	{
		...
		size_t tip_len = min(sizeof(data.szTip)-1, wcslen(name));
...
bool CTrayIcon::ShowBalloonTooltip(const TCHAR* title, const TCHAR* msg, ETooltipIcon icon)
{
	...
	size_t title_len = min(sizeof(data.szInfoTitle)-1, wcslen(title));
	wmemcpy(data.szInfoTitle, title, title_len);
	data.szInfoTitle[title_len] = 0;

	size_t msg_len = min(sizeof(data.szInfo)-1, wcslen(msg));
	wmemcpy(data.szInfo, msg, msg_len);
