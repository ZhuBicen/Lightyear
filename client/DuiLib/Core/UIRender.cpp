...
HBITMAP CRenderEngine::GenerateGrayBitmap(CPaintManagerUI* pManager, CControlUI* pControl, RECT rc)
{
    int cx = rc.right - rc.left;
    int cy = rc.bottom - rc.top;
    ...
    //// int iBytesScanLine = iBytesScanLine = ((4 * cx + 3)>>2)<<2;
    //int iBytesScanLine = iBytesScanLine = ((4 * cx + 3)>>2)<<2;  
    ...
    //    for (int i=0;i<cx;++i)  
    //    {  
    //        pBitsCurrent = (BYTE*)(pDest + j * iBytesScanLine + i * 3);  
    //        b = pBitsCurrent[0];  
    //        g = pBitsCurrent[1];  
    //        r = pBitsCurrent[2];  
